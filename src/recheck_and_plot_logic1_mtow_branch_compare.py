...
DEFAULT_AUTO_FULL_ROOT = PROJECT_ROOT / "avl_optimize_portable" / "runs_logic1_mtow_avl_recheck_branch_compare"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mlp-dir", default="gen_mlp_logic1_fixedpoint_mtow")
    p.add_argument("--avl-dir", default="gen_avl_logic1_fixedpoint_mtow")
    p.add_argument("--outdir", default="analysis_logic1_mtow_avl_recheck_branch_compare")
    p.add_argument("--runs-dir", default="runs_logic1_mtow_avl_recheck_branch_compare")
    p.add_argument("--kk-base", type=int, default=8200000)
...
def load_best(branch_dir: Path) -> tuple[int, int, dict[str, float], dict[str, float]]:
    ...
    best_idx = int(info_df["mtow_out"].astype(float).idxmin())
...
def metric_line(prefix: str, row: dict[str, float], info: dict[str, float], feasible: bool | None = None) -> str:
    text = (
        f"{prefix}: mtow={float(info['mtow_out']):.0f}, cy={float(info['cy']):.3f}, "
        f"K={float(info['K']):.2f}, p0={float(row['p0']):.2f}, qT={float(info['q_g_per_ton_km']):.2f}, "
...
def main() -> None:
    ...
    for i, branch in enumerate(BRANCH_ORDER):
        ...
        avl_gen, avl_idx, avl_row, avl_info = load_best(avl_branch)
        mlp_gen, mlp_idx, mlp_row, mlp_info = load_best(mlp_branch)
        ...
        out_path = out_branch / f"{branch}_topview_logic1_mtow_avl_vs_mlp.png"
    ...
    summary_df.to_csv(outdir / "logic1_mtow_branch_compare_summary.csv", index=False)

    fig, axes = plt.subplots(6, 2, figsize=(16, 34))
    axes_arr = axes.flatten()
    fig.suptitle("Logic1 mtow | AVL best vs MLP best with AVL recheck", fontsize=18, y=0.995, x=0.01, ha="left")
